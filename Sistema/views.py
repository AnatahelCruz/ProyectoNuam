...
def login_view(request):
    if request.method == "POST":
        ...
        if login_attempt.is_blocked():
            tiempo_restante = (login_attempt.blocked_until - timezone.now()).seconds // 60
            messages.error(request, f"Cuenta bloqueada. Intenta nuevamente en {tiempo_restante} minutos.")
            return redirect("login")
        ...
        if user_auth is None:
            ...
            if login_attempt.attempts >= 5:
                ...
                messages.error(request, "Has excedido 5 intentos. Intenta nuevamente en 15 minutos.")
            else:
                remaining = 5 - login_attempt.attempts
                messages.error(request, f"Correo o contraseña incorrectos. Te quedan {remaining} intentos.")
        ...
        user_role = UserRole.objects.filter(user=user_auth).first()
        ...
        rol = user_role.role.nombre.lower()
